...
- 网页提取成功后自动学习网站配置和实际成功模式，新建同站任务时自动复用并优先尝试成功模式。
- 自动记录各网站的提取成功率、使用模式、候选数量和平均耗时，用户主动停止提取不计为失败。
- 状态栏实时显示总下载速度、本次运行下载量、活动任务数以及程序和深度提取进程的 CPU、内存占用。
...
网页提取成功后，程序按完整网站域名自动学习本次有效的提取配置和实际成功模式。以后新建同站网页任务时自动复用；智能模式会优先尝试上次成功的方式，同时保留另一种方式作为失败后备。提取失败和用户主动停止不会覆盖已经成功的配置。

自动保存内容仅限提取模式、代理、来源页、浏览器标识、登录信息、超时和网络重试。分片线程、下载限速和文件规则仍使用新任务当时的全局设置，已经存在的任务也不会被改动。登录信息沿用程序现有的 Windows 加密保护方式保存。

设置页可以查看和删除已保存的网站配置，也可以查看站点兼容性记录。兼容性记录在一次提取完成后更新，包含成功率、提取次数、最近使用模式、候选数量和平均耗时；深度模式内部自动重试以及智能模式回退只算一次最终结果，用户主动停止提取不计为失败。记录只保存在本机，可以随时清空。

窗口底部状态栏显示当前总下载速度、本次进程新增的下载量、正在下载和正在提取的父任务数。CPU 和内存统计覆盖主程序、深度提取工作进程及其 Chromium 子进程，资源数据每秒采样一次，避免统计本身频繁占用资源。
